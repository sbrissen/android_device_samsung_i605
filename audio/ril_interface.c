...
#define ALOG_TAG "ril-audio_hw_primary"
...
int ril_open(struct ril_handle *ril)
{
    ...
    _ril_open_client = dlsym(ril->handle, "QmiCsd"); //android::QmiCsd::QmiCsd();
    _ril_close_client = dlsym(ril->handle, "CloseClient_RILD");
    _ril_connect = dlsym(ril->handle, "csd_clinet_start_voice"); //android::QmiCsd::setStartVoice(void)
    _ril_is_connected = dlsym(ril->handle, "isConnected_RILD");
    _ril_disconnect = dlsym(ril->handle, "Disconnect_RILD");
    _ril_set_call_volume = dlsym(ril->handle, "SetCallVolume");   //android::QmiCsd::setVoiceVolume(int, float)
    _ril_set_call_audio_path = dlsym(ril->handle, "setVoicePath"); //android::QmiCsd::setVoicePath(int, int)
    _ril_set_call_clock_sync = dlsym(ril->handle, "SetCallClockSync");
    _ril_set_two_mic_control = dlsym(ril->handle, "SetTwoMicControl"); // android::QmiCsd::setDualMic(bool)
...
int ril_set_call_audio_path(struct ril_handle *ril, enum ril_audio_path path)
{
    if (ril_connect_if_required(ril))
    {
	ALOGI("ril_set_call_audio_path: ril_connect_if_required(true)");
        return 0;
    }
